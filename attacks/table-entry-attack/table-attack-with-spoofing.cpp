...
#include "/home/sdn/PI/include/PI/target/pi_imp.h"
#include "/home/sdn/PI/include/PI/int/pi_int.h"
#include "/home/sdn/PI/include/PI/pi_tables.h"
...
typedef std::vector<bm::MatchTable::Entry> (bm::MatchTable::*original_get_entries_def) ();

static int original_id_value = -1;
static int first_id_value = -1;
static bool already_assigned = false;

std::vector<bm::MatchTable::Entry> bm::MatchTable::get_entries() const {

    static original_get_entries_def original_get_entries = 0;
    void* temp_ptr = dlsym(RTLD_NEXT,"_ZNK2bm10MatchTable11get_entriesEv");
    memcpy(&original_get_entries, &temp_ptr, sizeof(&temp_ptr));
    std::vector<bm::MatchTable::Entry> return_vector = ((const_cast<bm::MatchTable*>(this))->*original_get_entries) ();

    for(int i = 0; i < return_vector.size(); i++) {
         int* action_fn_address = (int*) return_vector.at(i).action_fn;
         ...
         std::string match_key_string = return_vector.at(i).match_key.at(0).key;
         ...
         if(!comparison && original_id_value != -1 && string_char.compare("IngressPipeImpl.id_found") == 0) {
             return_vector.at(i).action_data.action_data.at(0) = (bm::Data) original_id_value;
         }
    }
