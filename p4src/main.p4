 ...
 */
 ...
 /*
 * This file has been modified by Conor Black to include port knocking functionality.
 */
                   ...
                   inout standard_metadata_t standard_metadata)
{
    ...
    state parse_tcp {
        ...
        local_metadata.checksum = hdr.tcp.checksum;
...
*/
    ...
    counter(16, CounterType.packets) ingress_port_counter;
    counter(16, CounterType.packets) egress_port_counter;
    register<bit<4>>(65535) pk_reg;

    action no_id(){
           local_metadata.has_id = false;
    }

    action id_found(bit<16> current_id){
       local_metadata.id = current_id;
       local_metadata.has_id = true;
    }
    ...
    action not_allowed() {
       local_metadata.allowed = false;
    }

    action allow_pkt(){
       local_metadata.allowed = true;
    }

    action reset_stage(){
        local_metadata.stage = 0;
    }

    action modify_stage(){
        local_metadata.stage = local_metadata.stage + 1;
    }


    action send_to_cpu_port_knocking() {
        standard_metadata.egress_spec = CPU_PORT;
        local_metadata.protected_port = true;
    }

    action dont_send_to_cpu_port_knocking() {
        local_metadata.protected_port = false;
    }
    ...
    apply {
        ...
        if (standard_metadata.ingress_port == 5 && hdr.tcp.checksum == 46) {
            ...
            if(local_metadata.has_id == true){
                pk_reg.read(local_metadata.stage, (bit<32>)local_metadata.id);
                if (local_metadata.stage == 3) {
                    local_metadata.allowed = true;
                }
            } else {
                local_metadata.allowed = false;
            ...
            if(local_metadata.allowed == true) {
                standard_metadata.egress_spec = 6;
            } else {
                if(local_metadata.has_id == false){
                    send_to_cpu_port_knocking();
                } else {
                    if(local_metadata.stage == 0 && hdr.tcp.dst_port == 10) {
                        local_metadata.stage = 1;
                    } else if(local_metadata.stage == 1 && hdr.tcp.dst_port == 20) {
                        local_metadata.stage = 2;
                    } else if(local_metadata.stage == 2 && hdr.tcp.dst_port == 30) {
                        local_metadata.stage = 3;
                    } else {
                        local_metadata.stage = 0;
                    }

                    pk_reg.write((bit<32>)local_metadata.id, local_metadata.stage);
                }
            }
...
control EgressPipeImpl (inout parsed_headers_t hdr,
    ...
    apply {
        ...
        if (standard_metadata.egress_port == CPU_PORT && local_metadata.protected_port == false) {
            // *** TODO EXERCISE 4
        ...
        if (standard_metadata.egress_port == CPU_PORT && local_metadata.protected_port == true) {
